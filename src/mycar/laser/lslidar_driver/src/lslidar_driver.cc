...
int angle_min_cut[]={0,180}; 	//雷达屏蔽角度，这里屏蔽角度为135°到225°，
                        //如果要多角度屏蔽，如10~30，50~60，改为：angle_min_cut[]={10，50};angle_max_cut[]={30，60};
int angle_max_cut[]={90,270};  //修改后编译即可
...
bool LslidarDriver::loadParameters() {
	...
	this->declare_parameter<int>("ceil_increase",0);
	...
	this->get_parameter("ceil_increase", ceil_increase);
...
void LslidarDriver::pubScanThread()
{
  ...
  while (rclcpp::ok())
  {
	...
	int scan_num = ceil((angle_able_max-angle_able_min)/360*count_num)+ceil_increase;            //n10 +0; m10-1
        //printf("count_num = %d\n",count_num);
    ...
    if(truncated_mode_==1){
        int len=sizeof(angle_max_cut) / sizeof(angle_max_cut[0]);
		for (int j = 0; j < len; ++j) {
		if ((point_idx >= (angle_min_cut[j] * count_num / 360) ) && (point_idx <= (angle_max_cut[j] * count_num / 360 ))) {
		    scan->ranges[point_idx] = std::numeric_limits<float>::infinity();
    ...
    if(scan->intensities[point_idx]<=6 && scan->intensities[point_idx]>0){
		    scan->ranges[point_idx] = std::numeric_limits<float>::infinity();
		    scan->intensities[point_idx] = 0;
